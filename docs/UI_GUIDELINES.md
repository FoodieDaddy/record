...
- 正文透明度不低于 `0.40`；辅助中文不低于 `0.32`，且不以低透明度隐藏关键信息
...
允许：细描边、切角按钮、弱扫描线、HUD 小标签、状态点、数据条、甲板透视线、全息扫描环、数字等宽。

禁止：大面积发光、所有文字变蓝、按钮光晕、持续粒子、控制台日志堆砌、英文副标题、英文 kicker、装饰性英文翻译、大量英文缩写、AI 长段解释、模拟游戏战斗 UI。

中文标题字距克制；辅助标签保持易读，不用字距和低透明度制造装饰性噪声。
...
- 选中只显示中文舱位名；状态点 `#00C8FF` + 弱光晕；底部能量条青蓝渐变
...
运行时主品牌统一使用「太空记分器」。Space Scorekeeper 仅用于独立品牌标识或工程说明，不作为中文标题的英文副标题。

海报可独立使用 `SPACE SCOREKEEPER` 品牌标识，不附加英文舱位副标题。不再使用 SMART RECORD 或 PULSE TERMINAL。
...
- 页面标题使用「导航舱」；主按钮使用「开始导航计算」，不附加英文 kicker
...
镜像图片：独立 Canvas 绘制，非页面截图，一次性绘制不循环重绘，Canvas 文案须 sanitize；英文品牌如需出现，使用独立的 `SPACE SCOREKEEPER` 标识。预览弹窗：保存镜像卡、发送给朋友、关闭预览。
...
规范：纯 CSS/WXML 图标；按钮只显示中文主文案，不附加英文副标题；能量线（底部青色弱光条）；状态点（右上角青色圆点）；按压反馈 `border-color` 0.18→0.55 不做 scale；reduce-motion `transition: none !important`；按钮数量 2 个并排等宽。
