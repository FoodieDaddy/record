...
基地总控台是面向管理员的桌面端 Web 后台，用于管理太空记分器小程序的所有数据和系统状态。视觉风格为「飞船控制中心」— 黑底、蓝光、HUD 面板、三级面板体系、克制发光。
...
- 左侧：品牌标识 + SVG 轨道图形（120 秒旋转动画）
- 右侧：账号密码表单
- 交互：输入聚焦发光、登录按钮切角、错误 Toast 提示
- 入场：左右面板分层淡入
- 按钮文案：「接入基地总控台」
- 错误提示统一为「接入失败，请检查账号或权限」

### 基地总览 Dashboard

- 基地状态总览条（系统健康指示灯 + 上次同步时间 + 倒计时 + 刷新按钮）
- 6 个核心指标卡（按优先级：活跃编队 > 今日脉冲 > 新增用户 > 封存航程 > 航段写入 > 脉冲总量）
- 基地态势 SVG 图（编队节点 + 连接线 + 航船点）
- 系统健康矩阵（MySQL/Redis 实测连通性 + 延迟）
- 脉冲趋势折线图（近 30 天，可展开/收起）
- 实时事件流（最近 10 条，按类型着色）
- 30 秒自动刷新 + 倒计时提示
...
|---|---|
| 全局背景 | 径向蓝光 + 网格线 + 扫描线叠加 |
| 一级面板 | 半透明深色底 + 蓝色描边（页面主内容） |
| 二级面板 | 更透明底 + 弱描边（图表/筛选区） |
| 三级面板 | 透明底 + 极弱描边（状态/标签） |
| 面板头部 | 蓝色底色渐变 |
| 表头 | 大写 + 蓝色底色 + 固定定位 |
| 表格行 hover | 弱蓝背景 |
