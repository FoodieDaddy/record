...
- WebSocket `/ws/score` 以空间为广播单位，推送 `TRANSFER`、`SCORE_UPDATE`、`MEMBER_JOIN`、`MEMBER_LEAVE`、`ROUND_*`、`SETTINGS_CHANGED`、`SETTLE` 等事件。
- 创建空间、成员关系、用户设置、镜像档案、策略日志、身份等级进入 MySQL；封存和本局录生效会写 MySQL；趋势、身份、镜像主要从历史归档数据计算。
- 黑匣子样本在工程层主要对应 `room.all_record`、`room_member.final_score`、历史归档、趋势统计和策略/镜像/身份的输入样本。
...
- `CacheWarmUpRunner` 会从 MySQL 活跃空间重建 Redis 元信息、房间号映射和排行榜。
- `RoomTimeoutTask` 每 5 分钟扫描 3 小时无活动空间。

## 硬约束

- 后端按 2 核 2G 容器预算设计。
- 严禁引入高频全空间轮询、无差别广播、阻塞式批量计算、运行期大对象 JSON 反复序列化等 CPU/内存放大逻辑。
