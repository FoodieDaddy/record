...
- 对于重新校准测验终端，显示题目的屏幕卡片在页面垂直方向上必须居中对齐（使用 `justify-content: center` 且不使用内联 `padding-top` 避让）。底部的倾向水晶按键必须明确显示中文字样：“是”和“否”。题目生成摒弃以前的卡片分页形式，必须重构为终端控制台打字机流式日志输出模式（换行一个字一个字生成），且只显示题目文字本身，移除系统日志字样、题目序号和用户选择结果。顶部进度条使用 20 段精密倾斜切角的能量格子式进度条进行步进呈现。`scroll-view` 必须彻底隐藏右侧滚动条（使用组件专属配置与 `::-webkit-scrollbar { display: none; }` 双保险），并在底部渲染 Spacer 垫高占位，使每次打字机输出和换行置底时，当前题能够精准保持在卡片屏幕的垂直正中间，历史已答题目文字以 28% 低对比度淡化为背景，并向上平滑滚动。
