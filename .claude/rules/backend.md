...
- 策略输出的 `noiseLevel` 字段遵循 `LOW-NOISE` / `MEDIUM-NOISE` / `HIGH-NOISE` 三级体系。
- `LOW-NOISE` 表示编队状态稳定、记录节奏正常。
- `MEDIUM-NOISE` 表示编队存在波动、需要注意节奏调整。
- `HIGH-NOISE` 表示编队状态异常、需要暂停或重新校准。
- 前端展示时使用中文映射：低噪 / 中噪 / 高噪。
