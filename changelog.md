...
- 指令 / 星图校准中央星系主视觉重构：主星核与近轨次级行星尺寸放大 30%~60% 增强饱满度，并新增 sphere 立体阴影、柔和发光与边缘球面反光层，使行星拥有精致体积感；提供偏蓝、偏蓝绿、偏冷白等差异化色相；引入 1 条断续的辅助预测轨道以增强轨道丰富度。
- 星图计算视觉内容深度增强：新增行星间极其微弱的“路径推演”预测线，配合 CSS 流光在连线上产生动态滚动的采样流光点；远轨小节点增加 Lighthouse 信标采样脉冲动画（csFarPulse）；轨道系统在校准态下绑定 core-system 扫描频率自动触发扫掠提亮效果（csOrbitHighlight）；主核额外包裹一层 dotted 点阵校准环，极大提升了计算质感。
- “开始校准”主操作按钮重设计：改用更精致的胶囊实体形态，配合深蓝黑半透明背景、极细青蓝描边与低亮呼吸，增强飞船控制台设备感；内部加入 btn-scan-light 横向扫描微光与 btn-energy-line 底部能量线，并在 idle 下徐徐扫过、按压时快速滑过、starting 启动时激射闪烁，提供高度逼真的硬件级物理回馈。
- 指令 / 星图校准页重构为“极简 UI + 随机星图动画系统”：静态元素大幅简化，仅保留顶部小状态（● 待机、● 校准中、● 已完成）、中央多行星星图、简洁主副标题和最核心的操作按钮；已完成状态的结果投影区改为极简卡片面板，移除了原本复杂的参数卡片、进度条 and 英文HUD小字，提升视觉留白。
